...
class BasePatchExtractor(ABC):
    ...
    def extract_patches(self,
        ...
        tissue_mask = self.tissue_detector.detect_tissue(img,
                                                         img_mpp,
                                                         )
        self.remove_small_components(tissue_mask,
                                     img_mpp=img_mpp,
                                     )
        ...
        if self.max_area is not None:
            ...
            scale = min(1,
                        np.sqrt(self.max_area / img_area))
        else:
            scale = 1

        if scale != 1:
            downscaled_dims = (round(img.shape[1] * scale),
                               round(img.shape[0] * scale)
                               )
            tissue_mask = cv2.resize(tissue_mask,
                                     downscaled_dims,
                                     interpolation=cv2.INTER_NEAREST,
                                     )

        patches = self._extract_patches(tissue_mask,
                                        scale,
                                        **kwargs,
                                        )
        if scale != 1:
