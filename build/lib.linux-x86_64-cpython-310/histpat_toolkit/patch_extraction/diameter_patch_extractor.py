...
class DiameterPatchExtractor(BasePatchExtractor):
    ...
    def _extract_patches(self,
        ...
        num_of_components, labels, _, _ = cv2.connectedComponentsWithStats(mask.astype(np.uint8),
                                                                           connectivity=4,
                                                                           )

        def furthest_point(graph,
                           start,
                           ):
        ...
        for i in range(1, num_of_components):
            ...
            distance = cv2.distanceTransform(
                points.astype('uint8'), cv2.DIST_L2, 5)
            distance[distance != 0] = distance.max() + 1 - \
                distance[distance != 0]

            graph = tcod.path.SimpleGraph(
                cost=distance.astype('int64'), cardinal=1, diagonal=1)
            ...
            for i in range(1, len(path)):
                if i + 1 == len(path) or np.linalg.norm(path[i] - current) > rect_height:
                    ...
                    anchor = current - rect_width / 2 * \
                        np.array([np.sin(angle), np.cos(angle)])
                    rectangles.append(Rectangle(anchor[1], anchor[0], rect_width, min(
                        rect_height, np.linalg.norm((dx, dy))), -angle))
