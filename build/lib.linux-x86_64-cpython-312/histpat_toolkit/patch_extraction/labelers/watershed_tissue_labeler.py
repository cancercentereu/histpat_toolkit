...
class WatershedTissueLabeler(BaseTissueLabeler):
    ...
    def label_by_anchors_mask(self,
        ...
        """
        _, markers = cv.connectedComponents(anchors_mask,
                                            connectivity=4,
                                            )

        # WARNING This may produce some artifacts which we remove later on
        markers = cv.watershed(cv.merge((tissue_mask,) * 3),
                               markers,
                               )
        ...
        for label in range(1, markers.max() + 1):
            if (markers == label).any():
                ...
                _, comp_markers, stats, _ = cv.connectedComponentsWithStats(label_mask,
                                                                            connectivity=4,
                                                                            )
