...
class DZIFile:
    ...
    def size_at_level(self, level):
        return (self._length_at_level(self.width, level), self._length_at_level(self.height, level))
    ...
    def rounded_tiles_at_level(self, level):
        width, height = self.size_at_level(level)
        return (max(1, round(width / self.tile_size)), max(1, round(height / self.tile_size)))
