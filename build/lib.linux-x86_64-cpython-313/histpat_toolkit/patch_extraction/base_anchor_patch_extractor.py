...
class BaseAnchorPatchExtractor(BasePatchExtractor):
    ...
    def _extract_patches(
        ...
        modified_mask = self.tissue_modifier.modify_tissue(
            tissue_mask,
            scale=scale,
        )
        anchors_mask = self.tissue_anchorizer.anchorize_tissue(
            modified_mask,
            scale=scale,
        )
        labels = self.tissue_labeler.label_by_anchors_mask(
            anchors_mask,
            tissue_mask,
            scale=scale,
        )
        ...
        contours = [
            cv.findContours(
                ((labels[min_y : max_y + 1, min_x : max_x + 1] == label) * 255).astype(np.uint8),
                cv.RETR_EXTERNAL,
        ...
        return self.tissue_rectangularizer.rectangularize_tissue(
            contours,
            scale=scale,
        )
